...
def get_swing_levels(df: pd.DataFrame, lookback: int = 5):
    """Legacy backward-compatible swing function."""
    recent_closed = df.iloc[-(lookback + 1):-1]
    swing_low = float(recent_closed["low"].min())
    swing_high = float(recent_closed["high"].max())
